...
const flattenCart = (cart: User['cart']): User['cart'] => ({
    ...
    items: cart?.items
        ...
        .filter(Boolean) as CartItem[] as any,
});
...
export const CartProvider = (props: any) => {
            ...
            const syncCartFromLocalStorage = async () => {
                ...
                if (parsedCart?.items && parsedCart?.items?.length > 0) {
                    const initialCart = await Promise.all(
                        parsedCart.items.map(async ({ product, quantity }: { product: Product, quantity: number }) => {
    ...
    useEffect(() => {
        ...
        if (!hasInitialized.current || user === undefined || !cart?.items)
            return;
        ...
        if (user) {
            ...
            if (
                JSON.stringify(flattenCart(user.cart)) ===
                JSON.stringify(flattenedCart)
            ) {
    ...
    useEffect(() => {
        ...
        const newTotal =
            cart?.items?.reduce((acc: number, item: { product?: string | Product | null | undefined; quantity?: number | null | undefined; id?: string | null | undefined; }) => {
                return (
                    acc +
                    (typeof item.product === 'object'
                        ? JSON.parse(item?.product?.priceJSON || '{}')
                            ?.data?.[0]?.unit_amount *
        ...
        setTotal({
            formatted: formattedPrice(newTotal/100),
            raw: newTotal,
        });
    }, [cart, hasInitialized]);
